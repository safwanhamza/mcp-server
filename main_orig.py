...
# ---------- FastAPI ----------
app = FastAPI(title="Veo 3 Fast – Video Generation Pipeline (continuity from best tail frame GPT+Gemini)")
...
def extract_frames_exact_192(in_video: str, out_dir: str):
    ...
    _run_ffmpeg([ "-i", in_video, "-vf", f"fps={TARGET_FPS}", "-vsync", "0", pattern ])
...
def extract_audio(in_path: str, out_audio_path: str):
    _run_ffmpeg(["-i", in_path, "-vn", "-ac", "2", "-ar", "48000", "-c:a", "aac", "-b:a", "128k", out_audio_path])
...
def concat_audio_to_m4a(audio_paths: List[str], out_path: str):
    ...
    _run_ffmpeg(["-f", "concat", "-safe", "0", "-i", tmp_list, "-c:a", "aac", "-b:a", "128k", out_path])

def encode_video_from_frames(frames_dir: str, out_path: str, fps: int = TARGET_FPS):
    _run_ffmpeg(["-framerate", str(fps), "-i", os.path.join(frames_dir, "%06d.png"), "-c:v", "libx264", "-pix_fmt", "yuv420p", "-r", str(fps), out_path])
...
async def generate(
    ...
    product_image_file: UploadFile = File(None),
    ...
    # Step 1: Create the first reference image using Gemini
    prompt = build_prompt(
        brand=brand.strip(),
        product=product.strip(),
        scene_key=scene_setting,
        presenter_key=presenter_style,
        is_first=True
    )
    reference_image_path = os.path.join(out_dir, "first_reference_image.png")

    if gemini_upscale_image(first_ref_url, reference_image_path):
        next_image_url = upload_public_png(reference_image_path, f"{S3_PREFIX}/{job_id}/first_reference_image.png")
    else:
        next_image_url = first_ref_url
    ...
    JOBS[job_id] = {
        ...
        "req": {
            "brand": brand.strip(),
            "product": product.strip(),
            "product_image_url": next_image_url,
